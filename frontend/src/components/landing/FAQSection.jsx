      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Accordion type="single" collapsible className="space-y-4">
          {data.questions.map((item, index) => (
            ...
            >
              <AccordionTrigger className="px-6 py-4 text-left text-[#012578] font-semibold hover:no-underline hover:bg-[#1CAF35]/5 transition-colors">
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 bg-[#1CAF35]/10 rounded-lg flex items-center justify-center flex-shrink-0">
                    <HelpCircle className="w-4 h-4 text-[#1CAF35]" />
                  </div>
                  <span className="text-left">{item.question}</span>
                </div>
              </AccordionTrigger>
              <AccordionContent className="px-6 pb-4">
                <p className="text-[#012578]/70 leading-relaxed pl-11">
                  {item.answer}
                </p>
              </AccordionContent>
            </AccordionItem>
