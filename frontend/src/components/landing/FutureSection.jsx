  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="grid gap-4">
            {data.highlights.map((highlight, index) => (
              ...
              >
                <p className="text-3xl md:text-4xl font-bold text-[#FFFFFF] mb-1">
                  {highlight.value}
                </p>
                <p className="text-[#FFFFFF]/70">{highlight.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Growth Chart Visual */}
        <div className="mt-12 bg-[#1CAF35]/20 rounded-3xl p-8">
          <div className="flex items-end justify-center gap-4 h-32">
            {[20, 35, 50, 70, 100].map((height, index) => (
              <div
                key={index}
                className="w-12 md:w-16 bg-gradient-to-t from-[#1CAF35] to-[#FFFFFF] rounded-t-lg transition-all duration-500"
                style={{ height: `${height}%` }}
              />
            ))}
          </div>
          <div className="flex items-center justify-center gap-2 mt-4">
            ...
            <p className="text-[#FFFFFF] font-medium">Crescimento do seu negócio</p>
          </div>
        </div>
      </div>
    </section>
